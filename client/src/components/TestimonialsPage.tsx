...
const STORIES = [
  {
    ...
    detail:
      "Nhờ phòng mổ hybrid và AI dự báo biến chứng, ca thay van chỉ kéo dài 95 phút. Chú được gắn thiết bị ghi nhận nhịp tim liên tục và đội ngũ theo dõi từ xa 24/7.",
    rating: 4.9,
    metrics: [
  ...
  },
  {
    ...
    detail:
      "Kết hợp AI đọc MRI và hội chẩn đa chuyên khoa, phẫu thuật bảo tồn được thực hiện cùng robot hỗ trợ. Sau mổ, chương trình tâm lý trị liệu và dinh dưỡng đồng hành liên tục.",
    rating: 5,
    metrics: [
  ...
  },
  {
    ...
    detail:
      "Ứng dụng HealthCare+ Kids nhắc thuốc, đo phổi tại nhà và gửi cảnh báo khi chỉ số lệch chuẩn. Gia đình nhận tư vấn dinh dưỡng, bài tập hít thở qua video call hàng tuần.",
    rating: 4.8,
    metrics: [
...
export function TestimonialsPage({ onNavigate }: TestimonialsPageProps) {
  ...
  return (
    <div className="min-h-screen py-12 px-4 sm:px-6 lg:px-8 bg-slate-950/60">
      <div className="max-w-5xl mx-auto space-y-8">
          ...
          {CATEGORIES.map((cat) => (
            <button
              ...
              className={`px-4 py-1.5 rounded-full border text-sm transition ${
                filter === cat ? "bg-blue-600 text-white border-blue-600" : "border-slate-700 text-muted-foreground"
              }`}
            >
              {cat}
            </button>
        ...
        <div className="grid gap-6">
          {filteredStories.map((story) => (
            <Card key={story.id} className="p-6 border-slate-800/50 bg-gradient-to-br from-slate-950 to-slate-900">
              ...
              <div className="grid sm:grid-cols-3 gap-3 mt-4">
                {story.metrics.map((item) => (
                  <div key={item.label} className="rounded-xl border border-slate-800 px-3 py-2 text-sm">
                ...
                <Dialog>
                  ...
                  <DialogContent className="max-w-lg">
                    ...
                    <div className="grid grid-cols-2 gap-3 mt-4">
                      {story.metrics.map((item) => (
                        <Card key={item.label} className="p-3 bg-muted/40">
