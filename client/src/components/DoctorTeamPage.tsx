...
interface DoctorTeamPageProps {
  onNavigate: (page: string) => void;
}

export function DoctorTeamPage({ onNavigate }: DoctorTeamPageProps) {
      ...
      <div className="max-w-6xl mx-auto space-y-6">
          ...
          {departments.map((dept) => (
            <button
              ...
              className={`px-4 py-1.5 rounded-full border text-sm transition ${
                department === dept ? "bg-blue-600 text-white border-blue-600" : "border-slate-700 text-muted-foreground"
              }`}
            >
        ...
        <div className="grid md:grid-cols-2 gap-5">
          {doctors.map((doctor) => {
            ...
            return (
            <Card key={doctor.id} className="p-5 border-slate-800/50 bg-gradient-to-br from-slate-950 to-slate-900">
              ...
              <div className="flex flex-wrap gap-2 mt-4">
                {doctor.achievements.slice(0, 3).map((ach) => (
                  <Badge key={ach} variant="outline" className="text-xs">
                    {ach}
                  </Badge>
                ))}
              </div>
